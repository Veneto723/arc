...
const VERBS = 'switch|restart|delegate|mode|stance|add-account|add|remove-account|rm-account|remove|delete-account|del-account|rename|export|import|delete|peek|usage|trash|restore|notes|note|role|join|retire|help|arc';
...
function slashArgOk(verb, arg) {
  ...
  // project we discussed` took "for" as an account id, and because a matched prompt is ERASED, the
  // human's actual message was destroyed in the same stroke. That is the worst shape a false
  ...
  // Still open by design: `note` and `delegate`, whose argument IS free text. Gating them
