...
Everything you need for an ordinary turn is in THIS file. A companion file sits next to it for the one
moment that needs more, and it is named again at the point it applies:

| read | when |
|---|---|
| **handoff.md** | you are handing work off and peer / subagent / do-it-yourself is not obvious |
...
Any peer can also leave a one-line gotcha for whoever works here next: `arc fieldguide "<lesson>"`
(**→ handoff.md**).
...
**A peer is not a subagent.** A subagent is a fresh one-shot with no context that perishes when it
answers — right for a bounded question. A peer is a live session that already has the history and
keeps it — right for a thread someone should own. Delegating to an empty chair opens a window with
