...
// Priority: arc-config.json poolDb.neonUrl > legacy scripts/pool-config.json.
...
module.exports = function poolNeonUrl() {
  ...
  try {
    const cfg = JSON.parse(fs.readFileSync(path.join(home, '.claude', 'arc-config.json'), 'utf8'));
